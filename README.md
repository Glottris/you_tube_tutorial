...
We need to start an emulator or connect a phone.
if you already have en emulator installed you can run one of thease oprions
...
4. This will open a dialog box, here press create new virtual device.
5. Here select a device to create, I have used Nexus5X.
6. Press `next` and download the needed apis.
7. Press `next` again and select a name and orientation and press `Finish`


Now that we have our code running in the emulator we need to setup our hot reload so we can get instant feedback when we make changes.

Start a new console window in the same place and run `npm start` 

If it don't work you might need to fix some settings, 
to enable hot reload in an emulator run 
```
> adb shell input keyevent 82 (or shake phone?)
// and press enable hot reloade
```
if you still get some error read it, I had to run this 
```
